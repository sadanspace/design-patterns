 ...
 *
 *  迭代二:
 *  使用抽象工厂模式改进，每增加一个新配置，则需要添加三个类文件，例如，.ini
 *  IniConfigParserFactory
 *  IniRuleConfigParser
 *  IniSystemConfigParser
 *
 */
...
public class Config {
    public Config load(String ConfigFilePath, String type){
        ...
        if (type.equalsIgnoreCase("rule")) {
            parser = ConfigParserFactoryMap.getParserFactory(ConfigFileExtention).createRuleConfigParser();
            if (parser == null){
            ...
            parser = ConfigParserFactoryMap.getParserFactory(ConfigFileExtention).createSystemConfigParser();
            if (parser == null){
